essim_spearman <- function(phy, trait, nsim = 1000, is) {
	...
	# Spearman's correlation between splits statistic and trait
	res <- cor.test(is, trait, method="spearman", exact = FALSE)
	...
	# Spearman's correlations of simulated datasets
	sim.r <- sapply(1:nsim, function(x) cor.test(is[as.vector(rownames(sims))], sims[,x], method="spearman", exact = FALSE)$estimate)
